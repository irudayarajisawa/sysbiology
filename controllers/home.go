...
import (
	...
	"github.com/emadera52/sysbiology/crypto"
	"github.com/emadera52/sysbiology/models"
)
	...
	if sess != nil {
		...
		if usrNm != nil {
			...
			if err := user.Read("Username"); err == nil {
				...
				// Create "sysbiology" session initialized with user DB values
...
func (hc *HomeController) Register() {
	...
	if hc.Ctx.Input.Method() == "POST" {
		...
		// Create "sysbiology" session and init with shared values
		sm := make(map[string]interface{})
		sm["username"] = user.Username
		sm["fullname"] = user.Fullname
...
func (hc *HomeController) Login() {
	...
	if hc.Ctx.Input.Method() == "POST" {
		...
		// Create "sysbiology" session, init with user session values, return to home
		sm := make(map[string]interface{})
		sm["email"] = emAddr // unencrypted email address
		sm["username"] = user.Username
		sm["fullname"] = user.Fullname
...
func (hc *HomeController) Logout() {
	hc.DelSession("sysbiology")      // delete current session
	hc.Data["InSession"] = ""   // indicate that user has not registered
